...
public:
	...
	Car(str car_name, str car_model, str GOS_number, vec baggage) : car_name_(car_name), car_model_(car_model), GOS_number_(GOS_number),
	 baggage_(baggage) {
		std::cerr << "ctor param" << std::endl;
	}
	Car(const Car& p) : car_name_(p.car_name_), car_model_(p.car_model_), GOS_number_(p.GOS_number_), baggage_(p.baggage_) {
